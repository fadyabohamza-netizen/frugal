...
export function tierForBody(body: ResponsesBody): { tier: Tier; is_long: boolean } {
    ...
    const asksReasoning = /\b(prove|proof|derive|architecture|synthesi[sz]e|algorithm|rigorous|formal|explain why|step-by-step|design a)\b/i.test(text);
    const asksLongForm = /\b(essay|long-form|comprehensive report|detailed analysis)\b/i.test(text);
    const asksCode = /```|\bfunction\b|\bclass\b|\bimport\b|\bconst\b|\bdef\b/.test(text);
    const isLong = len > 1800;

    let tier: Tier;
    if (isLong || greedy || asksReasoning || asksLongForm) tier = "DEEP";
    else if (isContinued || images > 0 || hasTools || asksCode || len > 400) tier = "BALANCED";
    else tier = "FAST";
...
export function inputTokensFor(body: ResponsesBody): number {
    const input = body.input;
    const chars = typeof input === "string" ? input.length : flatText(input).length;
...
async function getCatalog(pollinations: AgentContext["pollinations"]): Promise<CatalogModel[]> {
...
async function getHealth(pollinations: AgentContext["pollinations"]): Promise<Map<string, HealthRow>> {
    ...
    for (const row of j.data) {
        if (row.is_rollup === 1 && row.event_type === "generate.text" && !map.has(row.model)) {
...
export default async function agent({ request, pollinations }: AgentContext): Promise<Response> {
    ...
    console.log(
        JSON.stringify({ router: "frugal", tier: picked.tier, model: picked.model, reason: picked.reason }),
    );
